...
public class MenuMgn extends JMenuBar implements ActionListener {
	...
	private JMenuItem stdAdd;		// 학생 추가
	private JMenuItem stdDel;		// 학생 삭제
	private JMenuItem stdUpdate;	// 학생 수정
	...
	public void actionPerformed(ActionEvent e) {
		if(e.getSource() == stdAdd) {
			...
			String stdId = JOptionPane.showInputDialog(null, "삭제하실 학생의 ID를 입력하세요");
			if(stdId != null) {
				...
				if(std.getIdx() != 0) {
					...
					if(confirmDel == 0) {
						if(dao.deleteDao(Integer.parseInt(stdId))==0) {
							...
							JOptionPane.showMessageDialog(null, "삭제가 실행 되지 않았습니다.");
						}
					}					
				}				
			}			
		}else if(e.getSource() == stdUpdate) {
			String stdId = JOptionPane.showInputDialog(null, "수정하실 학생의 ID를 입력하세요");
			if(stdId != null) {
