...
public class DepartInsertUpdate extends AbsInsertUpdate {	
	...
	protected void updateInit() {
		...
		if(depart.getCode() !=0) {
			...
			comboProf.setSelectedItem(depart.getProf());   // 추후 수정
	...
	protected Object getObject() {
		if(obj == null) {				// 추가			
			...
			depart = new Depart(Integer.parseInt(lbl_departCode.getText()), jtf_departName.getText(),
					(int)comboProf.getSelectedItem(), txtTel.getPhone());
			return depart;
		}else {							// 수정
			depart = (Depart) obj;
			...
			// 추후 수정
			depart.setProf((int)comboProf.getSelectedItem());
	...
	protected JPanel getMainPanel() {
		...
		// 추후 수정 
		ArrayList<Object>arProf = daoTable.selectDao();			
		int len = arProf.size();		
		comboProf = new JComboBox<>();
		for(int i=0;i<len;i++) {
			Depart depart = (Depart)arProf.get(i);
			comboProf.addItem(depart.getProf());
		}
